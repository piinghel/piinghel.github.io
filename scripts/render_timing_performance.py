...
def render(rows: list[dict[str, str]], output: Path, *, dark: bool) -> None:
    ...
    with plt.rc_context(
        {"font.size": 11, "svg.fonttype": "none", "svg.hashsalt": "timing-performance"}
    ):
        ...
        fig.legend(
            handles,
            ["Week 1", "Week 2", "Week 3", "Three-sleeve mixture"],
            loc="lower center",
            ncol=4,
            frameon=False,
            labelcolor=ink,
        ...
        fig.subplots_adjust(left=0.085, right=0.975, top=0.96, bottom=0.19)
...
def main() -> None:
    ...
    for dark in (False, True):
        render(
            rows,
            assets / f"schedule-performance{'_dark' if dark else ''}.svg",
            dark=dark,
        )
