...
MODEL_ORDER = ("fixed_factor_benchmark", "ols_c0", "selected_c0p1")
MODEL_LABEL = {
    "fixed_factor_benchmark": "Fixed weights",
    "ols_c0": "OLS",
    "selected_c0p1": "Ridge c = 0.1",
}
MODEL_COLOR = {
...
ALPHA_MODELS = (
    ...
    "alpha_scaled_c0p001",
    "alpha_scaled_c0p01",
    "alpha_scaled_c0p1_selected",
)
ALPHA_LABELS = ("OLS\nc = 0", "Ridge\n0.001", "Ridge\n0.01", "Ridge\n0.1")
SPLIT_DATE = date(2021, 1, 1)
...
def load_alpha_diagnostics(
    ...
    sensitivity: dict[str, dict[str, dict[str, str]]] = defaultdict(dict)
    for row in read_rows(review_dir / "multiple_linear_alpha_sensitivity.csv"):
        if row["model"] in ALPHA_MODELS:
            sensitivity[row["model"]][row["period"]] = row
    coefficients = {
        row["model"]: row
        for row in read_rows(
            review_dir / "multiple_linear_alpha_coefficient_diagnostics.csv"
        )
        if row["model"] in ALPHA_MODELS
    }
    for model in ALPHA_MODELS:
        if set(sensitivity[model]) != {"pre_2021", "2021_2026", "full"}:
            raise ValueError(f"incomplete period evidence for {model}")
        if model not in coefficients:
            raise ValueError(f"missing coefficient diagnostics for {model}")
    return sensitivity, coefficients


def load_selected_coefficients(review_dir: Path) -> list[dict[str, str]]:
    rows = read_rows(
        review_dir / "multiple_linear_selected_coefficient_heatmap_source.csv"
    )
    selected = [row for row in rows if int(row["heatmap_rank"]) <= 10]
    if len(selected) != 120:
        raise ValueError("expected ten selected predictors across twelve folds")
    if any(float(row["c"]) != 0.1 or row["selected"].lower() != "true" for row in selected):
        raise ValueError("coefficient heatmap must use the selected c=0.1 model")
    return selected


def load_selected_portfolio_tilts(review_dir: Path) -> list[dict[str, str]]:
    rows = read_rows(
        review_dir / "multiple_linear_selected_portfolio_tilt_figure_source.csv"
    )
    ...
    if any(float(row["c"]) != 0.1 or row["selected"].lower() != "true" for row in rows):
        raise ValueError("portfolio tilts must use the selected c=0.1 model")
    return rows
...
def plot_ic(series: dict[str, Series], output_dir: Path, *, mobile: bool) -> None:
    ...
    for model in MODEL_ORDER:
        ...
        ax.plot(
            ...
            linewidth=2.0 if model == "selected_c0p1" else 1.6,
        )
        offset = {"fixed_factor_benchmark": -10, "ols_c0": 2, "selected_c0p1": 10}[
            model
        ]
        ax.annotate(
            MODEL_LABEL[model],
            ...
            color=MODEL_COLOR[model],
            fontsize=9,
            fontweight=600 if model == "selected_c0p1" else 400,
            va="center",
        )
    style_axis(ax)
...
def plot_performance(
    ...
    *,
    ...
    for model in MODEL_ORDER:
        width = 2.0 if model == "selected_c0p1" else 1.5
        wealth_ax.plot(
            wealth[model].dates,
            wealth[model].values,
            color=MODEL_COLOR[model],
        ...
        offset = {"fixed_factor_benchmark": -10, "ols_c0": 0, "selected_c0p1": 10}[
            model
        ]
        wealth_ax.annotate(
            MODEL_LABEL[model],
...
def plot_alpha_sensitivity(
    ...
    *,
    ...
    for period, label, color, marker in (
        ("pre_2021", "Development through 2020", OLS, "o"),
        ("2021_2026", "Later period", RIDGE, "s"),
    ):
        values = [
            float(sensitivity[model][period]["mean_daily_rank_ic"])
            for model in ALPHA_MODELS
        ]
        deltas = [1000 * (value - values[0]) for value in values]
        axes[0].plot(
            x, deltas, color=color, marker=marker, linewidth=1.7, markersize=4.5
        )
        axes[0].text(
    ...
    axes[0].set_ylabel(
        "Change in mean daily IC vs OLS (×10⁻³)", color=MUTED, fontsize=9
    )

    prediction_corr = [
        100 * float(sensitivity[model]["full"]["prediction_rank_corr_alpha_0"])
        for model in ALPHA_MODELS
    ]
    axes[1].plot(
    ...
    ols_l2 = float(coefficients[ALPHA_MODELS[0]]["coefficient_l2_mean"])
    ols_change = float(
        coefficients[ALPHA_MODELS[0]]["adjacent_fold_mean_abs_change_mean"]
    )
    l2 = [
        100 * float(coefficients[model]["coefficient_l2_mean"]) / ols_l2
        for model in ALPHA_MODELS
    ]
    change = [
        100
        * float(coefficients[model]["adjacent_fold_mean_abs_change_mean"])
        / ols_change
        for model in ALPHA_MODELS
    ]
    ...
    axes[2].set_title(
        "Coefficient shrinkage and movement",
        loc="left",
        color=INK,
        fontsize=10.5,
        fontweight=600,
...
def main() -> None:
    ...
    ic = load_daily_series(
        review_dir / "multiple_linear_alpha_daily_ic_figure_source.csv",
        value_column="daily_rank_ic",
    )
    wealth = load_daily_series(
        review_dir / "multiple_linear_alpha_return_drawdown_figure_source.csv",
    ...
    sensitivity, coefficients = load_alpha_diagnostics(review_dir)
