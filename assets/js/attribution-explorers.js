...
(() => {
  ...
  const session=byId('recovery-session');
  ...
  function chart(svg,root,rows,series,values,index){
    ...
    for(const [col,key,dash] of series){
      ...
      const d=rows.map((r,i)=>{
        ...
        const part=!active?`M${x(i)},${y(r[col])}`:`L${x(i)},${y(r[col])}`;
    ...
    for(const [i,anchor] of [[0,'start'],[Math.floor((rows.length-1)/2),'middle'],[rows.length-1,'end']]){
      add(svg,'text',{x:x(i),y:height-6,'text-anchor':anchor},`Session ${i}`);
    }
  }
  ...
  fetch('/assets/portfolio-attribution/explorer-paths.json?v=5').then(r=>{
    ...
    inspectCharts(detail,session,renderRecovery);
    new ResizeObserver(renderRecovery).observe(recovery);
    new MutationObserver(renderRecovery).observe(document.documentElement,{attributes:true,attributeFilter:['data-theme']});
    updateRecovery();
  }).catch(()=>{
