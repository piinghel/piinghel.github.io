...
defensive factor favours low volatility and avoids stocks whose recent gains
depend on a few sharp up days. Momentum looks for a trend that remains after
skipping the latest month. The other factors favour lower short positioning,
larger companies, and fewer negative-return days.

I first turn each measure into a percentile rank among the stocks available on
the same date, with higher ranks always meaning more attractive stocks. Write
these ranks as $$L$$ for low volatility, $$U$$ for upper-tail avoidance, $$M$$
for momentum, $$S$$ for low short positioning, $$Z$$ for company size, and
$$C$$ for return consistency. For stock $$i$$ on date $$t$$, the defensive
factor $$D$$ averages its two inputs:

$$
D_{i,t}=\frac{L_{i,t}+U_{i,t}}{2}.
$$

The benchmark then averages the five final factors:

$$
F_{i,t}=\frac{D_{i,t}+M_{i,t}+S_{i,t}+Z_{i,t}+C_{i,t}}{5}.
$$

Ranks put unlike measures such as volatility and company size on one common
scale. Each final factor receives 20%. Because $$L$$ and $$U$$ split the
defensive factor, each contributes 10% to the benchmark score.

The raw measures use volatility over roughly one, three, and six months; the
three largest daily gains in the latest month; momentum over three to twelve
months after skipping the latest month; short interest relative to trading
volume; market value; and the share of negative-return days over three years.

Short-positioning data are lagged until they would have been available, and a
stock needs enough history for all five factors before it can enter the
benchmark.

The five factors are meant to bring different information into the score, but
they need not be independent. Figure 1 measures their same-date cross-sectional
rank correlation during development. I match all five final scores on the same
5,587,838 stock-date rows, including the combined defensive score, calculate a
Spearman correlation on each date, and average across 6,195 dates. The lower
triangle shows each pair once; purple is negative, blue is positive, and the
number in each cell gives the correlation.
...
<p class="figure-caption"><strong>Figure 1:</strong> Mean same-date Spearman rank correlation among the five final fixed factors, 21 May 1997–31 December 2021. The defensive score is the final combination of low volatility and upper-tail avoidance.</p>

Figure 1 shows some overlap, but no strong relationship. The largest displayed
correlation is 0.28, between defensive and return consistency; defensive and
short positioning are slightly negative at −0.08. No single factor is a close
substitute for another.

Table 1 asks a different question: what happens when each final factor is used
...
three rebalance schedules, and reports annualized gross return through 2021,
...
<p class="table-caption"><strong>Table 1:</strong> Development-period annualized arithmetic gross return for each standalone factor portfolio, averaged across the three full-capital rebalance schedules.</p>

All five standalone portfolios have a positive gross return in development.
Defensive leads at 7.14%, followed by return consistency at 5.64%; size is the
weakest at 0.65%. These results describe each factor on its own. They do not
show how much a factor adds once the other four are already in the model.
...
Figure 2 keeps those stages separate: raw measures become comparable ranks, the
...
<p class="figure-caption"><strong>Figure 2:</strong> Signal-combination pipeline. Fixed and learned combinations feed the same stock-selection and portfolio rules.</p>

The separation in Figure 2 matters for the comparison: only the
...
block—a walk-forward design. Figure 3 shows the sequence. The first 900 dates
...
<p class="figure-caption"><strong>Figure 3:</strong> Expanding walk-forward estimation, with a 21-day buffer before each 600-date prediction block.</p>

The expanding windows in Figure 3 protect each prediction block from future
...
and 21-day gap in Figure 3 prevent the most direct leakage across prediction
...
Table 2 gives the first test of the penalty choice. It compares portfolio return
...
Table 2 does not show a decisive Ridge improvement. The moderate $c=0.01$
...
Figure 4 asks what that moderate penalty actually changes. Panel A measures how
...
<p class="figure-caption"><strong>Figure 4:</strong> Average portfolio-membership changes and coefficient shrinkage relative to matched OLS during the development period.</p>

Figure 4 shows that coefficients change much more than the portfolio. At
...
Figure 5 follows the ten largest Ridge coefficients across refits. Positive
...
<p class="figure-caption"><strong>Figure 5:</strong> Signed coefficients for the selected Ridge model's ten largest mean absolute weights across walk-forward refits; 2022 and 2024 show later refits.</p>

A few broad directions remain stable in Figure 5: price relative to its moving
...
Table 3 asks whether the broader learned model improves portfolio results. It compares
...
Table 3 shows where the broader OLS model differs from the benchmark.
...
I selected $c=0.01$ using data through 2021. Table 4 repeats the portfolio
...
In Table 4, the selected Ridge model does not beat OLS after 2021. Its
...
Figure 6 accumulates those daily correlations through time. A rising path means the
...
<p class="figure-caption"><strong>Figure 6:</strong> Cumulative daily cross-sectional Spearman information coefficient for the fixed score, OLS, and selected Ridge predictions; the rule marks the 2022 boundary.</p>

The OLS and Ridge paths in Figure 6 are almost indistinguishable. Table 5
...
Table 5 confirms what the overlapping paths suggest. During development, Ridge
...
At first, the fixed score looks different: Table 5 gives it the highest
...
Figure 7 shows when the portfolio differences accumulated. The upper
...
<p class="figure-caption"><strong>Figure 7:</strong> Net growth of <span class="mathjax-ignore">$1</span> and drawdowns for the three ranking systems after charging 5 bp per dollar traded. The upper panel uses a logarithmic scale; the rule separates development from the later period.</p>

Figure 7 shows that the broader learned models' development-period
...
advantage may be expensive to trade. Figure 8 compares two-way turnover and the
...
<p class="figure-caption"><strong>Figure 8:</strong> Average two-way turnover per rebalance and annual trading cost under the 5 bp assumption, shown separately for development and the later period.</p>

Figure 8 shows that the fixed score trades about half as much as either learned
...
the portfolio. Figure 9 tracks the selected Ridge portfolio's long gross, short
...
<p class="figure-caption"><strong>Figure 9:</strong> Monthly average floating exposures for the selected Ridge portfolio.</p>

Figure 9 shows the long book staying near its ceiling while the short book does
...
negative values mean the reverse. Figure 10 averages these tilts by quarter.
...
Most of the ten largest realised tilts in Figure 10 fall into two broad groups.
...
<p class="figure-caption"><strong>Figure 10:</strong> Quarterly paths of the ten largest average absolute realised predictor tilts; panels use their own zero-inclusive scales and label the full-sample mean.</p>

Figure 10 shows a portfolio that is mainly defensive and trend-following. The
...
choices. Figure 10 cannot separate those effects.
