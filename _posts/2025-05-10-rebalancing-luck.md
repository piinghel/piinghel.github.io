...
<p class="article-summary">Splitting a three-week strategy into three tranches leaves its average return almost unchanged, with less depending on the rebalance calendar. In the later period, the annualized return spread falls from 5.13 percentage points across fifteen standalone calendars to 1.06 across five three-tranche portfolios. Average volatility falls from 9.31% to 8.80%.</p>
...
8.02% return is close to their 7.99% average. The benefit is reducing dependence
on the chosen week, rather than capturing the best-to-worst return gap.
...
why I can't separate them so neatly. Week 3 returns 10.55% with Monday signals
and 5.42% with Friday signals: the best offset on Monday becomes the worst on
Friday. Within four of the five weekdays, changing the starting week produces
a return spread of 3.64–4.49 percentage points. Thursday is the exception,
with a spread of only 0.67 points.
...
Changing the weekday while keeping the offset fixed also produces sizeable
spreads: 2.79, 3.75 and 5.13 points for Weeks 1, 2 and 3. A descriptive
offset–weekday decomposition assigns most of the variation to their interaction
in both periods. The calendar combination matters; this grid doesn't identify
a starting week that works consistently across weekdays. Its shared returns
also do not provide fifteen independent observations.
...
The five combined portfolios still range from 7.25% to 8.31% annualized return.
...
<div class="research-figure rebalancing-figure responsive-figure">
  {% include theme-svg-figure.html base="/assets/tranching/calendar-return-risk" mobile="/assets/tranching/calendar-return-risk_mobile" version="2" alt="Across fifteen standalone calendars and five three-tranche portfolios, mean annualized return changes from 7.87% to 7.91% and mean volatility falls from 9.31% to 8.80%. Each weekday shows the same pattern. January 2022 to May 2026." %}
</div>
...
Table 3 keeps the same comparison across the full experiment. Average net return
moves from 7.87% across the fifteen standalone calendars to 7.91% across the five
combined portfolios. Average volatility falls from 9.31% to 8.80%, a reduction of
0.51 percentage points, or 5.5%. The average Sharpe rises from 0.86 to 0.91.
The return barely changes; lower volatility drives the improvement.
...
I would fund all three tranches. The later-period comparison preserves almost
the same average return, removes about four fifths of the calendar spread, and
lowers portfolio volatility for every weekday. Those benefits survive the
existing 5 bp costs. For me, that is enough to justify maintaining three books.
The weekday still matters, but much less of the outcome rests on one arbitrary
calendar.
