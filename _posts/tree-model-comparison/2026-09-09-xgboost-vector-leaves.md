...
I also included Ridge from my earlier article as a benchmark, alongside LightGBM as another tree model. Each predicts forward Sharpe—the ratio of future mean daily return to volatility—ranked within date and sector. All four use the same 144 predictors, training windows and eligible stocks, with forecasts evaluated after their training windows.[^setup]
...
I picked up this idea from [Numerai](https://docs.numer.ai/community/office-hours-with-arbitrage), where model evaluation looks at both average scores and their variability across eras. Here I use the **information coefficient (IC)**, the daily cross-sectional Spearman correlation between forecasts and their targets. Its mean measures average ranking quality; its standard deviation measures how much that quality varies across dates.

The **IC information ratio (ICIR)** divides the mean IC by that sample standard deviation, left unannualized. Figure 1 separates these quantities over four five-year periods for the blends, evaluated against the mean of the two ranked targets on common dates. I start in 2002 to leave out the initial partial period.
...
Do these changes show up in the portfolio? Figure 2 compares ranking quality with portfolio performance for LightGBM and Ridge; the three tree blends are so close that one is enough here. Each labelled point represents one five-year period, using the same dates for the forecast and portfolio measures. Forecast measures are on the horizontal axes, and higher points mean better portfolio outcomes. The first pair of plots compares Sharpe with ICIR. The next two pairs compare annual return and Sharpe with the standard deviation of daily IC, so farther right means more variable ranking quality. The filled triangles mark the final period.

<div class="research-figure responsive-figure" markdown="0">
{% include theme-svg-figure.html base="/assets/tree-model-comparison/forecast-outcomes" mobile="/assets/tree-model-comparison/forecast-outcomes_mobile" version="3" alt="Three pairs of plots for the LightGBM and Ridge 50:50 blends: annualized Sharpe versus ICIR, annual net arithmetic return versus SD of daily IC, and annualized Sharpe versus SD of daily IC. Forecast measures are on the horizontal axes and portfolio outcomes on the vertical axes. Four labelled periods per model; filled triangles highlight 2017–2021. LightGBM's final period has greater IC variability and lower return and Sharpe than 2012–2016." %}
</div>
